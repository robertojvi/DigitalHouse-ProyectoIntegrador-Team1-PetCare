  ...
  try {
    ...
    if (response.data) {
      if (typeof response.data === "object") {
        // Directly use response.data if it has id_categoria
        if (response.data.id_categoria) {
          categoryData = response.data;
        }
        // Try common wrapper structures
        else if (response.data.data && response.data.datid_categoriaia) {
          categoryData = response.data.data;
        } else if (
          response.data.categoria &&
          response.data.categoria.id_categoria
        ) {
          categoryData = response.data.categoria;
        }
        // Try to find any object with id_categoria or id
        else {
          for (const key in response.data) {
            if (
              response.data[key] &&
              (response.data[key].id_categoria || response.data[key].id)
            ) {
          ...
          if (!categoryData && (response.data.id || response.data._id)) {
            categoryData = {
              id_categoria: response.data.id || response.data._id,
              nombre:
                response.data.nombre || response.data.name || "Nueva Categoría",
            };
          }
        }
      }
    ...
    if (!categoryData) {
      console.warn(
        "Could not find category data in response, using full response as fallback"
      );
      // Create a fallback object based on what we sent
      const parsedData = formData.get("datos")
        ? JSON.parse(formData.get("datos"))
        : {};
